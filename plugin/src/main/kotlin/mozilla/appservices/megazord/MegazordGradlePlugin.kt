...
open class MegazordPlugin : Plugin<Project> {
    ...
    override fun apply(project: Project) {
        with(project) {
            ...
            // This is somewhat temporary: https://github.com/mozilla/application-services isn't
            // publishing to maven.mozilla.org yet, so we need a non-standard URL.  But we probably
            // want to force https://maven.mozilla.org/maven2 in the future anyway... it's probably
            // not worth making this configurable.
            project.repositories.maven {
                it.url = URI.create("https://dl.bintray.com/ncalexander/application-services")
            }

            // This is delicate.  We need to match components _after_ dependency resolution in order
            // to substitute for transitive components... but after dependency resolution is to late
            // to perform substitutions.  Therefore we make a (recursive, excluding parent
            // configurations) copy and resolve that to examine, before substituting in the main
            // configuration.
            //
            // We must wait for project evalution because the Android-Gradle plugin doesn't create
            // its configurations until then.
